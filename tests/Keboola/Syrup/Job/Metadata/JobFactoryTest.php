...
class JobFactoryTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testJobFactory()
    {
        ...
        $objectEncryptor = new ObjectEncryptor(new CryptoWrapper($key));

        $jobFactory = new JobFactory(SYRUP_APP_NAME, $encryptor, $objectEncryptor);
     ...
     * @covers \Keboola\Syrup\Job\Metadata\JobFactory::setStorageApiClient
     */
    public function testJobFactoryDecryptParams()
    {
        $storageApiClient = new Client([
            'token' => SYRUP_SAPI_TEST_TOKEN,
            'userAgent' => SYRUP_APP_NAME,
        ...
        $command = uniqid();
        $param = ["key1" => uniqid(), "key2" => uniqid("encrypted")];

        $paramToEncrypt = $param;
        $paramToEncrypt["#key2"] = $param["key2"];
        unset($paramToEncrypt["key2"]);

        $lock = uniqid();
        $tokenData = $storageApiClient->verifyToken();

        $job = $jobFactory->create($command, ['param' => $objectEncryptor->encrypt($paramToEncrypt)], $lock);
